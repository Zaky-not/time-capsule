...
export default function FeaturedPhoto({ photo, align = 'center', onOpen }) {
  const alignClasses = ALIGN[align] ?? ALIGN.center;

  return (
    <div className={`w-full ${alignClasses} md:w-3/4 lg:w-2/3`}>
      <motion.button
        type="button"
        data-cursor="VIEW"
        onClick={onOpen}
        ...
        className={`film-frame group ${FEATURED_RATIO} block w-full overflow-hidden bg-surface`}
      >
        <img
          src={photo.src}
          alt=""
